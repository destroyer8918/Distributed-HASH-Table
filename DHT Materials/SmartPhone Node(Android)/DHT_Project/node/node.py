...
def should_store_key(key):
    """Android Node: Store all remaining keys based on MOD 10 rule."""
    try:
        numeric_key = int(key)  # Ensure the key is numeric
        should_store = numeric_key % 10 >= 5  # Store keys not handled by Linux/Mac
        print(f"Android Node: Key: {key}, Numeric Key: {numeric_key}, Should Store: {should_store}")
        return should_store
    except ValueError:
        # Non-numeric keys are stored by Android Node
        print(f"Android Node: Key: {key} is non-numeric. Storing.")
        return True

def forward_request(node_ip, node_port, command, visited_nodes):
    """Forward the request to another node."""
    try:
        # Add visited nodes to the command
...
def handle_client(client_socket):
    ...
    try:
        ...
        if command['type'] == 'store':
            key = command['key']
            value = command['value']
            if should_store_key(key):  # Check Android Node logic
                storage[key] = value
                save_data()  # Save to file after storing
...
def start_server(port):
    ...
    server.listen(5)
    print(f"Android Node started on port {port}. Waiting for connections...")
