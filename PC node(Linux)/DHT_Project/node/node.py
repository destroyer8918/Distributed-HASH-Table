...
def should_store_key(key):
    """Linux Node: Store keys based on MD5 even last digit."""
    hash_value = hashlib.md5(key.encode()).hexdigest()
    last_digit = hash_value[-1]
    numeric_value = int(last_digit, 16)  # Convert to base-16 integer
    should_store = numeric_value % 2 == 0  # Check if the numeric value is even
    print(f"Linux Node: Key: {key}, MD5 Hash: {hash_value}, Last Digit: {last_digit} (Numeric: {numeric_value}), Should Store: {should_store}")
    return should_store
